...
stimFreq=75/4;                                     % Frequency of stimulus pulse in hz
reinforcedArm=2;                                % Choose an arm to reinforce with light pulse
...
while toc < exp_duration
        ...
        if pixDev(mod(ct,10)+1)<8
            ...
            occupyingLitArm=choiceArm==reinforcedArm;
            ...
            [stimCoords,activeStim,stim_tStamps]=...
                getStimTargets(arm_coords,choiceArm,changedArm,activeStim,occupyingLitArm,tElapsed,mazeOri);
            
            % Convert stim coordinates from camera to projector coords
            stimCoords=[xfit(stimCoords(:,1)) yfit(stimCoords(:,2))];
            ...
            cenID=cat(3,cenID,lastCentroid');
            oriID=cat(1,oriID,tElapsed);
            turnID=cat(1,turnID,turnArm);
        end
