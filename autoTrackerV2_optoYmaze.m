...
[stimProps,screenNumber]=optoInitializeCheckerStim;
...
currentID = [handles.fpath '\' t strain '_' treatment '_current_arm.dat'];         % File ID for turn data
...
dlmwrite(currentID, []);                         % create placeholder ASCII file
...
while toc<referenceTime&&get(handles.togglebutton11,'value')~=1
    ...
    previous_tStamp=current_tStamp;
        ...
        [lastCentroid,centStamp,tOriDat]=...
            optoMatchCentroids2ROIswithori(cenDat,oriDat,centers,speedThresh,distanceThresh,lastCentroid,centStamp,tElapsed);
...
choiceArm=zeros(size(ROI_coords,1),1);          % Last arm a choice was made to
...
% stimFreq=75/8;                                     % Frequency of stimulus pulse in hz
reinforcedArm=1;                                % Choose an arm to reinforce with light pulse
ON_stamps=nan(size(activeStim));
saveCenPrev=centers;
saveOri=nan(size(ROI_coords,1),1);
tElapsedPrev=0;
dir_change=nan(size(ROI_coords,1),1);
dir_change2=nan(size(ROI_coords,1),1);
speed=nan(size(ROI_coords,1),1);
saveOriPrev=zeros(size(ROI_coords,1),1);
%% Run Experiment
%shg
tic
while toc < exp_duration
        ...
        delay=delay/10000000000000000000;
        %pause(delay);
...
%                  dir_change=tOriDat-saveOri          %5
%                  saveOri=tOriDat;                    %5
%                  dir_change2=atan2d(lastCentroid(:,2)-saveCenPrev(:,2),lastCentroid(:,1)-saveCenPrev(:,1))-saveOriPrev;   %5
%                  saveCenPrev=lastCentroid;           %5
%                  saveOriPrev=dir_change2;           %5
%             end
            ...
            % Update the most recent arm the fly made a turn into
            
            choiceArm(changedArm)=current_arm(changedArm);
            
%             modeChange=mod(floor(tElapsed/900),2);                        % Will be true where time elapsed > 0.5*period
%  
%             if modeChange==0  
%                 Screen('FillRect', stimProps.window, [1 0 0], stimProps.windowRect);
...
%             end
            ...
            occupyingLitArm=choiceArm==reinforcedArm;     %1
%             size(occupyingLitArm(2:2:end))
%             size(occupyingLitArm(1:2:end))
            occupyingLitArm(2:2:end)=occupyingLitArm(1:2:end);
%             occupyingLitArm=ones(size(choiceArm));           %2
%             occupyingLitArm=tOriDat<20 & tOriDat>(-20);         %4
%              occupyingLitArm=speed>1;           %5
%              occupyingLitArm=speed>2;              %6
            %If I want to reinforce right or left turns in order to avoid
            %the flies subjective space estimation to play a role. I would
            %need choiceArm-previousArm and then if positive, lit the
            %choice Arm and otherwise nothing
            
            %{ 
             rightTur=rightTurns;               %3
             rightTur(isnan(rightTur))=0;       %3
            %Otherwise (changedArm&&~rightTurns) %3
             occupyingLitArm=(changedArm&~rightTur);    %3
             ON_stamps(occupyingLitArm)=tElapsed;   %3
             occupyingLitArm((tElapsed-ON_stamps)<2)=1; %3
            stim_arm=ones(size(ROI_coords,1),1);    %2
            stim_arm(mazeOri)=stim_arm(mazeOri)+3;  %2
            %}
            %{ 
            cenDat2=nan(size(ROI_coords,1),1);
            j=squeeze(j);
            j(isnan(j))=[];
            j
            cenDat
            ...
            cenDat2
            %} 
            % Select appropriate arm coordinates for stimulation

            [stimCoords,activeStim,stim_tStamps]=...
                getStimTargets(reinforcedArm,lastCentroid,arm_coords,choiceArm,changedArm,activeStim,occupyingLitArm,tElapsed,mazeOri);
            %I have to change the stimCoords in getStimTargets if I just
            %want to target the fly. Instead of targeting the tmp_armcoords
            %for defining stimCoords I get the fly centroids with lastCentroid          
            size(stimCoords)
            ...
            % Display new stimuli
%               [pulseON,stim_tStamps]=optoDispCheckerStimFreq(stimProps,stimCoords,pulseON,activeStim,stim_tStamps,stimFreq,tElapsed,cum_tStamps);
             [pulseON,stim_tStamps]=optoDispCheckerStim(stimProps,stimCoords,pulseON,activeStim,stim_tStamps,tElapsed);
            % Only write choice arm for the frame in which a fly made a
            % choice
            ...
            dlmwrite(currentID, current_arm', '-append');
        end
...
flyTracks.currentarm=dlmread(currentID);
