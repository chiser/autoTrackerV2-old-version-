...
%stimFreq=75/5;                                     % Frequency of stimulus pulse in hz
reinforcedArm=2;                                % Choose an arm to reinforce with light pulse
freqs=[75/16 75/8 75/4];
for trying=1:length(freqs)
%% Save labels and create placeholder files for data
stimFreq=freqs(trying);
...
while toc < exp_duration
        ...
        if pixDev(mod(ct,10)+1)<8
            ...
            [stimCoords,activeStim,stim_tStamps]=...
                getStimTargets(cenDat,arm_coords,choiceArm,changedArm,activeStim,occupyingLitArm,tElapsed,mazeOri);
